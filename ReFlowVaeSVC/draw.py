...
SAMPLE_MIN = (
    2  # 抽取的文件数量下限 / The lower limit of the number of files to be extracted
)
SAMPLE_MAX = (
    10  # 抽取的文件数量上限 / The upper limit of the number of files to be extracted
)
...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=root_dir
        + "/data/train/audio",  # 固定源目录为根目录下/data/train/audio目录
    ...
    for item in os.listdir(src_dir):
        ...
        if os.path.isdir(item_path):
            ...
            for subitem in os.listdir(item_path):
                ...
                if os.path.isfile(subitem_path) and any(
                    [subitem.endswith(f".{ext}") for ext in extensions]
                ):
                    subfiles.append(subitem)
        elif os.path.isfile(item_path) and any(
            [item.endswith(f".{ext}") for ext in extensions]
        ):
            files.append(item)
