...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_plms(
        ...
        def get_x_pred(x, noise_t, t):
            ...
            a_prev = extract(
                self.alphas_cumprod, torch.max(t - interval, torch.zeros_like(t))
            )
    ...
    def OnnxExport(
        ...
        if init_noise is None:
            x = torch.randn(
                (1, 1, self.mel_bins, cond.shape[2]), dtype=torch.float32
            ).cpu()
        else:
            x = init_noise
        pndms = 100
    ...
    def forward(self, condition=None, init_noise=None, pndms=None, k_step=None):
        ...
        n_frames = cond.shape[2]
        step_range = torch.arange(
            0, k_step.item(), pndms.item(), dtype=torch.long, device=device
        ).flip(0)
        plms_noise_stage = torch.tensor(0, dtype=torch.long, device=device)
        noise_list = torch.zeros((0, 1, 1, self.mel_bins, n_frames), device=device)
