...
class Unit2Wav(nn.Module):
    def __init__(
        ...
        self.diff_model = GaussianDiffusion(
            WaveNet(out_dims, n_layers, n_chans, 256), out_dims=out_dims
        )
...
class Unit2WavFast(nn.Module):
    ...
    def forward(
        ...
        if not infer:
            ddsp_loss = F.mse_loss(ddsp_mel, gt_spec)
            diff_loss = self.diff_model(
                ddsp_mel, gt_spec=gt_spec, k_step=k_step, infer=False
            )
            return ddsp_loss, diff_loss
        else:
            if gt_spec is not None and ddsp_mel is None:
