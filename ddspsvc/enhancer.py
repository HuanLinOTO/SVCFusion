...
class Enhancer:
    ...
    def enhance(
        ...
        # resample f0
        if (
            hop_size == self.enhancer_hop_size
            and sample_rate == self.enhancer_sample_rate
            and sample_rate == adaptive_sample_rate
        ):
            ...
            time_frame = (
                self.enhancer_hop_size / self.enhancer_sample_rate
            ) * np.arange(n_frames)
            f0_res = np.interp(
                time_frame, time_org, f0_np, left=f0_np[0], right=f0_np[-1]
            )
            f0_res = (
