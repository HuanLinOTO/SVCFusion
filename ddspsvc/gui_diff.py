    ...
    def update_model(self, ddsp_model_path, diff_model_path):
        ...
        if self.ddsp_model is None or self.ddsp_model_path != ddsp_model_path:
            if ddsp_model_path is not None and ddsp_model_path != "":
                self.ddsp_model, self.ddsp_args = load_model(
                    ddsp_model_path, device=self.device
                )
    ...
    def infer(
        ...
        with torch.no_grad():
            if self.ddsp_model is None:
                ...
                output, _, (s_h, s_n) = self.ddsp_model(
                    units, f0, volume, spk_id=spk_id, spk_mix_dict=dictionary
                )
            output = self.diff_model.infer(
                output,
                f0,
                units,
                volume,
...
class GUI:
                ...
                sg.Frame(
                    layout=[
                        [
                            sg.Text(
                                i18n(
                                    "扩散模型文件：.pt格式(自动识别同目录下config.yaml)"
                                )
                            )
                        ],
                        [
                        ...
                        [
                            sg.Text(i18n("扩散深度")),
                            sg.Input(key="k_step", default_text="100", size=18),
                        ],
                        [
                            sg.Text(i18n("扩散加速")),
                            sg.Input(key="diff_acc", default_text="10", size=18),
                        ],
                        [
                            sg.Text(i18n("扩散算法")),
                            sg.Combo(
                                values=self.diff_method_list,
                                key="diff_method",
                                default_value=self.diff_method_list[0],
                                enable_events=True,
                            ),
                        ],
                        [
                            sg.Checkbox(
                                text=i18n("不扩散安全区(加速但损失效果)"),
                                default=False,
                                key="diff_silence",
                                enable_events=True,
                            )
    ...
    def start_vc(self):
        ...
        self.svc_model.update_model(
            self.config.checkpoint_path, self.config.diff_project
        )
        self.start_stream()
