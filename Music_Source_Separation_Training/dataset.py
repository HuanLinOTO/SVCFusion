...
class MSSDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        try:
            ...
            print(
                "Loading songs data from cache: {}. If you updated dataset remove {} before training!".format(
            ...
            if self.dataset_type in [1, 4]:
                ...
                for instr in self.instruments:
                    ...
                    if type(data_path) == list:
                        ...
                        track_paths += sorted(
                            glob(data_path + "/{}/*.wav".format(instr))
                        )
                ...
                print(
                    "Unknown dataset type: {}. Must be 1, 2 or 3".format(
                        self.dataset_type
                    )
                )
                exit()
    ...
    def load_source(self, metadata, instr):
        while True:
            if self.dataset_type in [1, 4]:
                ...
                source = load_chunk(
                    track_path + f"/{instr}.wav", track_length, self.chunk_size
                )
            else:
    ...
    def load_aligned_data(self):
        ...
        for i in self.instruments:
            ...
            while attempts:
                source = load_chunk(
                    track_path + f"/{i}.wav", track_length, self.chunk_size
                )
                if np.abs(source).mean() >= self.min_mean_abs:  # remove quiet chunks
                    break
                attempts -= 1
